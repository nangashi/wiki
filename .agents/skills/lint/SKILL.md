...
**itコレクションの注意**: リファレンス型は1技術1ページに集約する方針のため、`LARGE` フラグを分割理由にしない。分割候補とするのは「複数の技術を1ページで説明している」「1つのサブトピックだけでページの過半を占める」場合のみ（`wiki/it/schema.md` の「ページ単位」参照）。逆に、既存技術ページに収まる短い事例ページは統合候補として積極的に検出する。
