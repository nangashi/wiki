...
このファイルは現在有効な設定・呼び出し・責務の契約と、その理解・変更判断に必要な理由を定義する。変更対象から実装・検証を探す場合は [保守案内](maintenance.md)。通常の記事作業では対象wikiの設定と操作手順を読み、この文書全文を毎回読む必要はない。

## 責務と正本

各wikiは記事の目的・粒度・採用・評価・公開条件が異なるため、運用定義を独立させる。検索・リンク解決・index生成の基盤を共有するため、同じリポジトリに置く。
