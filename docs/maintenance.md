...
| insightの評価・公開処理変更 | [評価プロトコル](../wiki/insight/references/evaluation-protocol.md)、[lint手順](../wiki/insight/workflows/lint.md) | `wiki/insight/tools/evaluation_state.py`、`evaluation_validator.py`、`publication.py`、`check.py` | insightテスト。rubric version・本文schema・hash・既存履歴・再開条件への影響を確認 |
