...
構造・重複・矛盾監査は共通lintの手順に従い、粒度と採否はinsightのschema・再利用性基準で判断する。以下の再評価・改善は既定動作として同じlint実行内で行う。評価モードの選択や再承認を要求しない。診断・提案のみの明示指定では、設計・記事・indexを変更せず、指定範囲の診断と所見を報告する。
...
- 全件が現行かつ記事・設計の組が完成条件を満たす: 再評価しない

追加オプションを要求せず、対象件数と実行規模を報告してそのまま同じ `$lint` 実行内で進める。`editor`やAstraが評価を補完・代行しない。

設計の有無・合格・記事とのhash対応も全記事で診断する。設計がない記事は、過去の評価や導入履歴に関係なく、同じlint実行内で[設計手順](design.md)に従い作成・独立レビューし、記事との照合・必要な修正・独立最終評価まで行う。設計変更は記事hashが同じでも再評価対象とする。設計基準変更は全設計のレビューと対応する記事評価を更新する。

記事基準変更時の全件再評価は維持する。記事評価の開始前に対象の設計と設計評価を用意する。設計不足の解消は、記事評価の必須項目キューとは別に全対象で行い、過去の記事単独評価が合格でも省略しない。作成・評価できなかった記事は理由と次の対象を残し、未完了として報告する。読み取り専用のcheck.pyは不足の検出のみを行い、設計や記事を生成しない。
...
各記事では、[評価プロトコルの改善と外部検証](../references/evaluation-protocol.md#改善と外部検証)に従い、必要な外部検証、1〜3項目の改善、新しい独立再評価を行う。改善担当は同プロトコルの委譲・直接処理条件で決める。利用者は途中で停止できる。停止された場合は処理済み件数、未処理件数、次の対象を残す。統合・削除・リダイレクト化、核心の大幅変更は個別確認を取る。

## 通常の修正

構造上の改善はERROR → WARNING → INFOの順に扱う。明白で局所的な非破壊修正はまとめて提案できる。削除や統合は自動実行しない。

CHECK-4/6を含む通常の改善点は、問題、根拠、差分レベルの推奨対応を一件ずつ提示する。監査範囲内の通常の本文・リンク修正は適用し、必要な独立評価まで進める。診断・提案のみの明示指定では変更しない。統合・分割・削除・リダイレクトは個別確認を取り、スキップ時は変更せず次へ進む。
