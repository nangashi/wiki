...
from evaluation_state import current_rubric_version, latest_evaluations
...
def approved_insight(root: Path, slug: str) -> bool:
    ...
    return bool(current and current.get("status") == "current" and
                current.get("rubric_version") == version and current.get("pass") is True)
