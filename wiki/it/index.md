...
- [[claude-code-managed-settings]] — Claude Codeの組織セキュリティ設定。managed-settings.json+MDM（Jamf）配布・1Password CLI連携・段階的展開（Lv0〜Lv4）
- [[ai-coding-eslint-constraints]] — AIコーディング品質を担保するESLint制約設計。eslint-plugin-boundariesによる依存方向強制・禁止ルール・結合テスト戦略
- [[adr]] — Architecture Decision Recordの構造と活用。コンテキスト・決定・結果の3要素で技術選定の前提を記録し、感情ではなく記録に基づく撤退判断を可能にする
...
- [[ai-agent-design]] — AIエージェントは業務列挙でなく役割から設計する。役割先行設計と1エージェント1役割の原則で品質を安定させる
- [[indirect-prompt-injection]] — 外部テキストに悪意ある指示を埋め込みエージェントに実行させる攻撃手法。入力の信頼度分離・Managed Settings・権限最小化の3対策
